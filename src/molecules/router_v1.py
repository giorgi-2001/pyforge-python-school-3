...
from .schemas import MoleculeAdd, valid_smiles
...
def add_molecule(molecule: MoleculeAdd):
    '''Adds a new record of a molecule to the database:
    - Requires a **molecule object**.
    - The **molecule object** must contain the **name** attribute,
      which is the trivial name of the molecule.
    - The **molecule object** must contain the **smiles** attribute,
      which is the SMILES expression of the molecule.
    - The **id** for the molecule is generated automatically.
    - The request returns the newly created
      **molecule dictionary** as the response.'''
...
def get_molecules_by_substructure(substructure: str):
    '''Retrieves all records of molecules that
      contain the provided substructure:
    - Requires passing a query **substructure**.
    - The substructure must be in a valid **SMILES** format.
    - Every molecule in the database that contains the
      **substructure** will be returned in the response.
    - If no such molecules exist in the database, an
      **empty list** will be returned.
    - If the provided substructure does not have a valid
      **SMILES** format, an HTTPException will be raised.
    '''
...
def get_molecule_by_id(id: int):
    ...
    - The request will attempt to find the molecule
...
@router.put("/{id}")
def update_molecule(id: int, molecule: MoleculeAdd):
    '''
    Updates record of a molecule:
    - Requires valid **molecule object** and molecule **id**.
    - The request will attempt to find the molecule
      based on the provided **id**.
    - If the molecule exists, it will be **replaced**
      by the provided **molecule dictionary**
...
def delete_molecule(id: int):
    ...
    - The request will attempt to find the molecule
      based on the provided **id**.
    - If the molecule exists, it will be **deleted**
      and the response will confirm the deletion.
    - If the molecule does not exist, an HTTPException will be raised.
    '''
    for index, mol in enumerate(molecule_db):
        if mol["id"] == id:
...
def get_molecules(skip: int = 0, limit: int | None = None):
    ...
    - Returns **slice** of the molecule database if
      any of the parameters is provided
    '''
...
async def upload_file(file: UploadFile):
    '''
    Takes a JSON file as an input and adds its content
      to the **molecule database**
